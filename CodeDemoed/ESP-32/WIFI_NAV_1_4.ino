/* Code running on ESP-32 for demo video 
Includes Navigation and Unser interface code 
Navigation Writen by Max Lesser (ML)
User interface code written by Jonatahn Pouluse (JP)
Some parts written by Joshua Samaniego (JS)
Ownership of code sections will be indicated using initials in prentisis above
section breaks indicated by 3 lines of stars. Everything within one section is attributet to the 
auther listed at the top of the section unless otherwise noted 

Code downloads infromation from server, and controlls motors for navigation. 
...
#include <Wire.h>         // I2C 
//#include <LIS3MDL.h>      // Compass not used due to sensor failure 
//#include <LSM6.h>         // Gyro and Accel as above 

//LSM6 imu;                 // Gyro and accel instance as above 
//***********************************************************************************************************//
//***********************************************************************************************************//
//***********************************************************************************************************//
// by JS 
...
int L_SE_CNT_GLOB = 0; // left indicator 
int R_SE_CNT_GLOB = 0; // right indicator 
int L_A_GLOB = 0;      // left ch a 
int R_A_GLOB = 0;      // right ch a  
...
float Gyro_Cal = 0.0;     // gyro calibration dummy var not used as GYRO sensor failed 
...
//***********************************************************************************************************//
//***********************************************************************************************************//
// Motor control and Navigation functions, by ML 
// note commented out print statements were used during debug 
...
void GoStraight(int Rotations)
{
  ...
  if (correction)     // correction not used due to isseues with left wheel shaft encoder 
  {
  ...
  // both motors forward at medium speed 
  Left_F(MED); 
  ...
  while ((R_SE_CNT_GLOB < Rotations))
  {
    ...
    if (correction) // correction not used due to left wheel SE issues 
    { 
  ...
  if(correction) // correction not used 
  {
...
void Stop()
{
  ...
  // dosen't fall out of loop. attempt to use I channel instead -> added count to reset loop if needed 
  attachInterrupt(Pin_LA, L_A_ISR, RISING);
...
{ 
   ...
   * NOTE: due to Shaft encoder faliure, the tracking part of this function did not work( can't track left wheel counts if you're not getting left wheel counts) 
   * moved to time based version instead 
   * Also: insuficient time to implement side detection on ESP 
   */
  ...
  { 
    // time based version to go around obstacle in a trianlge pattern 
    // stop 
    digitalWrite(Blade, LOW); // turning off blade for evasive manouver 
    Right_R(10); 
    Left_R(10); 
    delay(5000); 
    // turing out 
  ...
  evading = true; // had some issues with noise on the alert pin that would sometimes inadvertantly trigger obstacle detection. After obstacle detection trials we set this to only 
  // perform obstacle evasion once. (This issue was discovered the day before demo was due, so we had no time to look into it. it did not do this during bench tests) 
...
{ 
   ...
   *  
   *  RESLOVED: H-Bridge overheated
   *  switched to bigger H-bridges 
   */
  R_A_GLOB = 0; 
  //int turncnt = 0;
  ...
  {
    /* code for using GYRO to turn accuratly. did not work as GYRO would return faulty readings sometimes 
     *  based on itegrating Gyro values 
    ...
    */
    ...
    Serial.println(" "); 
    if (R_A_GLOB >= 140)      // from 84 -> 150; 140 -> 155; 84-> 90 -> 105 // 84 should be 90 deg turn  // 130 fine tune turning distance 
    {
...
void setup() {
  ...
  //***********************************************************************************************************//
  //***********************************************************************************************************//
  // Nav Setup, By ML ( lines followed by JS written by JS) 
  
    // digital output pins 
    // constant control logic output for motors (ML reassigned pins written by JS for use with new H-bridge)  
    
    pinMode(PinL_1, OUTPUT);  //JS

    pinMode(PinL_2, OUTPUT);  //JS

    pinMode(PinR_1, OUTPUT);  //JS

    pinMode(PinR_2, OUTPUT);  //JS
    ...
    // PWM pins (reconfigured for new H-bridge)  
    ledcAttachPin(12,1); //( PinL_F, L_Channel);left forward JS

    ledcAttachPin(13,2); //( PinL_R, R_Channel); left backwards JS 
    ...
    // reconfigured PWM from old H-bridge to new H-brige 
    ledcSetup(1,5000,8);//( L_Channel, freq, resolution); JS 
    ...
    pinMode(O_ALERT, INPUT); // obstacle alert as input 

    pinMode(Blade, OUTPUT); // blade control as output 

     

    Serial.begin(115200);

    // these ISRs used in functions instead of set in general, to prevetn excessive Interupts requests
    //attachInterrupt(Pin_LA, L_A_ISR, RISING);

    //attachInterrupt(Pin_RA, R_A_ISR, RISING);
    // GYRO/ IMU not used due to problems with the sensor 
    ...
    */
    //Gyro_Cal = imu.g.x; //Sum/50;    // X axis gyro reading when mower is still attempted calibration 
}
//***********************************************************************************************************//
//***********************************************************************************************************//
//***********************************************************************************************************//
...
void TaskRemoteControl( void * pvParameters ) {
  for (;;) {
    if (sendReceive.toInt() == 1) {
      // Send Firebase
      xSemaphoreTake(myMutex, portMAX_DELAY);
      if (Firebase.setString(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/battery", "high")) 
      {
        //Success
      ...
      // Get Firebase /MD9L4CUlaQfQuf8bqzOWjdblaQD2/start_status
      if (Firebase.getBool(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/start_status"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/0/latitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/0/longitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/1/latitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/1/longitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/2/latitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/2/longitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/3/latitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
      ...
      if (Firebase.getDouble(fbdo, "/MD9L4CUlaQfQuf8bqzOWjdblaQD2/default_lawn/locations/positions/3/longitude"))
      {
        xSemaphoreTake(myMutex, portMAX_DELAY);
...
//***********************************************************************************************************//
//***********************************************************************************************************//
// by ML, where JP added check for start status in while loop 
void loop() {
   ...
   */
   ...
   if (start_status == "0") 
   { 
    digitalWrite(Blade, LOW); // turn off blade when user stops mower 
   }
    ...
    if((LoopNum == 0) & (LegNum == 0))
    { 
      digitalWrite(Blade, HIGH); // start blade after setup is compleate 
    }
    // if we have compleated the area given: 
    if (LoopNum >= LoopsNeeded) 
    { 
      // NOTE: need to modify this with RTB and sleep code 
      Stop();     // stop the mower 

      // turn off all motors and the balde 
      digitalWrite(Blade, LOW);
      
      digitalWrite(PinL_1, LOW); 

      digitalWrite(PinL_2, LOW);
    ...
    GoStraight( rots); 
    //delay(2000); 
    if(start_status == "0"){ // checks start status JP for this if statement 
      Stop();
      break;
    }
